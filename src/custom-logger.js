...
module.exports = class Logger {
    constructor(options) {
        ...
        this.setLogLevel = (newLevel) => {
            ...
            const level = levels.includes(this.options.level)
                ? this.options.level
                : 'debug';
            switch (level) {
                case 'trace':
                    this.trace = function () {
                        this.output('trace', Array.from(arguments));
                    };
                case 'debug':
                    this.debug = function () {
                        this.output('debug', Array.from(arguments));
                    };
                case 'info':
                    this.info = function () {
                        this.output('info', Array.from(arguments));
                    };
                case 'warn':
                    this.warn = function () {
                        this.output('warn', Array.from(arguments));
                    };
                case 'error':
                    this.error = function () {
                        this.output('error', Array.from(arguments));
                    };
                case 'fatal':
                    this.fatal = function () {
                        this.output('fatal', Array.from(arguments));
                    };
            }
        };
